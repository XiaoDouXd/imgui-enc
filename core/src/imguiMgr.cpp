...
namespace XD::ImguiMgr {
    ...
    void init(SDL_Window* wSdl, VkSurfaceKHR& surf, int w, int h) {
        ...
        _inst->io->IniFilename = xdWndInitConf_confFileName;

        _inst->wHandle.Surface = surf;
        auto res = VulkanMgr::getPhyDev().getSurfaceSupportKHR(VulkanMgr::getQueueFamily(), surf);
        if (res != VK_TRUE) throw Exce(__LINE__, __FILE__, "XD::App: 不支持的物理设备");
    ...
    void presentFont() {
        ...
        // io.Fonts->AddFontFromMemoryTTF(
        //     (void*)XD::RC::CC_RC_FONTS_SOURCEHANSANSCN_NORMAL_OTF.data(),
        //     XD::RC::CC_RC_FONTS_SOURCEHANSANSCN_NORMAL_OTF.size(),
        //     32.0f,
        //     &fontConfig,
